...
void AttributeType::make_ustring(const wchar_t *value) {
    attr_free();
    if (value) {
        kind_ = Attr_UString;
        size_ = (unsigned)wcslen(value);
        u_.ustring = static_cast<wchar_t *>(malloc(size_ + 1));
        memcpy(u_.ustring, value, size_ + 1);
        u_.ustring[size_] = 0;
    } else {
        kind_ = Attr_Nil;
    }
...
void AttributeType::realloc_list(unsigned size) {
    ...
    if (req_sz > cur_sz) {
        ...
        memcpy(static_cast<void *>(t1),
               static_cast<void *>(u_.list),
               size_ * sizeof(AttributeType));
...
void AttributeType::insert_to_list(unsigned idx, const AttributeType *item) {
    ...
    memset(static_cast<void *>(t1 + idx), 0, sizeof(AttributeType));  // Fix bug request #4

    memcpy(static_cast<void *>(t1), u_.list, idx * sizeof(AttributeType));
    t1[idx].clone(item);
    memcpy(static_cast<void *>(&t1[idx + 1]),
           static_cast<void *>(&u_.list[idx]),
...
void attribute_to_string(const AttributeType *attr, AutoBuffer *buf) {
    if (attr->is_nil()) {
        ...
        printf("Not implemented interface to dict. method\n");
    ...
    if (cfg[off] == '\'' || cfg[off] == '"') {
        ...
        while (cfg[off] != '}' && cfg[off] != '\0') {
            if (string_to_attribute(cfg, off, &new_key)) {
                printf("%s", "JSON error: Wrong dictionary key\n");
                out->attr_free();
                return -1;
            }
            off = skip_special_symbols(cfg, off);
            if (cfg[off] != ':') {
                out->attr_free();
                printf("%s", "JSON error: Wrong dictionary delimiter\n");
                return -1;
            }
            off = skip_special_symbols(cfg, off + 1);
            if (string_to_attribute(cfg, off, &new_value)) {
                printf("%s", "JSON error: Wrong dictionary value\n");
                out->attr_free();
                return -1;
            }
        ...
        if (cfg[off] != '}') {
            printf("JSON error: %s", "Wrong dictionary format\n");
            out->attr_free();
            return -1;
        }
        off = skip_special_symbols(cfg, off + 1);

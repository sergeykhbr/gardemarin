...
void DisplaySPI::PostInit() {
    SPI_registers_type *SPI3 = spi_bar();
    uint16_t t1;
    ...
    system_delay_ns(125000000);     // > 120 ms
...
void DisplaySPI::starCounter(void *dev, int usec) {
    tim_cr1_reg_type cr1;
    TIM_registers_type *TIM = (TIM_registers_type *)dev;

    write32(&TIM->ARR, static_cast<uint32_t>(10*usec)); // resolution 100 ns
    write32(&TIM->CNT, static_cast<uint32_t>(10*usec)); // resolution 100 ns
...
uint8_t DisplaySPI::show_char(const uint8_t *arr,
                              uint16_t x,
                              uint16_t y,
                              uint8_t chr,
                              uint8_t size,
...
void DisplaySPI::clearLines(int start, int total, uint16_t clr) {
    size_t m = total * 320 * 2 / sizeof(buffer_);     // total times
    size_t n = total * 320 * 2 % sizeof(buffer_);     // the last
    uint32_t *buf32 = reinterpret_cast<uint32_t *>(buffer_);
